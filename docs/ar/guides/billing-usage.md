...
يمر `chat.send` الموثَّق بـ JWT عبر بوابة الحصة الشهرية نفسها، لكن مقابل نافذة **المستخدم بالكامل** (لا يحمل الاستدعاء أي API key). الرفض هو خطأ JSON-RPC برمز محدد من طرف التطبيق `-32006` (`QUOTA_ERROR`) ونفس رسالة رفض الحصة في REST. لا يوجد حد معدل لكل مفتاح في مسار RPC — فحدود المعدل مرتبطة بالمفتاح واستدعاءات RPC لا تحمل مفتاحًا. يمر `realtime.start` عبر بوابة الحصة الشهرية نفسها للمستخدم بالكامل (فتح جلسة بحصة مستنفدة يُرفض بـ `-32006`)؛ ويفحص `video.create` الحصة عند إنشاء المهمة.

## دفتر نقاط (مسبق الدفع، مع tier كشبكة أمان)

تمتلك المجموعات والمستخدمون محافظ نقاط (`ledger_accounts` — محفظة شخصية لكل
مستخدم ومجمّع لكل مجموعة). التسوية **بالنقاط أولًا**: تحاول كل طلبية مُحصاة
خصم `round(cost_usd × POINTS_PER_USD × مضاعف العضو)` من الحساب المُحاصص
(مجمّع المجموعة لمفاتيح المجموعة، وإلا المحفظة الشخصية) — تحديثات ذرّية مشروطة،
لا يمكن للخصومات المتزامنة تجاوز الرصيد. عندما لا تغطي المحفظة المبلغ، تستخدم
الطلبية حصة tier الشهرية كما في السابق (دفع لاحق، `points = NULL`). عندما
يُنفد **الاثنان**، تستجيب بوابة REST بـ **402 Payment Required**
(`insufficient_credits` / `payment_required`، `Retry-After` حتى نهاية الشهر)؛
يحافظ مسار RPC على شكل `-32006` `QUOTA_ERROR`.

يأتي `POINTS_PER_USD` من البيئة (الافتراضي 100 — نقطة واحدة = سنت واحد).
سطح الإدارة: `group.credits.topup` (admin المنصة)، `group.credits.allocate`
(admin المجموعة، تحويل ذرّي من المجمع إلى المحفظة)، `group.credits.balance`،
`ledger.self`؛ يبلّغ `billing.plan` عن `points_balance` و `points_per_usd`.
تحمل صفوف الاستخدام المُسوّاة من الدفتر `points` و `account_type`
(`user` | `group`).
