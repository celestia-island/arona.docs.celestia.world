...
ключа нет. `realtime.start` проходит через тот же месячный гейт квоты на пользователя
целиком (открытие сессии с исчерпанной квотой отклоняется с `-32006`);
`video.create` проверяет квоту при создании задачи.

## Реестр очков (предоплата, tier как страховка)

У групп и пользователей есть кошельки очков (`ledger_accounts` — личный
кошелёк на пользователя и пул на группу). Расчёт **очки-в-первую-очередь**:
каждый измеренный запрос пытается списать
`round(cost_usd × POINTS_PER_USD × множитель участника)` со списываемого
счёта (пул группы для групповых ключей, иначе личный кошелёк) — атомарные
условные обновления, параллельные списания не могут уйти в минус. Когда
кошелёк не покрывает сумму, запрос использует месячную квоту tier как
раньше (постоплата, `points = NULL`). Когда исчерпаны ОБА, REST-шлюз
отвечает **402 Payment Required** (`insufficient_credits` /
`payment_required`, `Retry-After` до конца месяца); RPC-путь сохраняет
форму `-32006` `QUOTA_ERROR`.

`POINTS_PER_USD` берётся из окружения (по умолчанию 100 — одно очко = один
цент). Поверхность управления: `group.credits.topup` (admin платформы),
`group.credits.allocate` (admin группы, атомарный перевод пул→кошелёк),
`group.credits.balance`, `ledger.self`; `billing.plan` сообщает
`points_balance` и `points_per_usd`. Строки использования, рассчитанные
из реестра, несут `points` и `account_type` (`user` | `group`).
