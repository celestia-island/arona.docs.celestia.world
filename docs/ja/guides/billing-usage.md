...
JWT 認証の `chat.send` は同じ月次クォータゲートを通過しますが、**ユーザー全体**のウィンドウに対してです（呼び出しは API key を持ちません）。拒否は実装定義コード `-32006`（`QUOTA_ERROR`）の JSON-RPC エラーで、REST クォータ拒否と同じメッセージです。RPC パスにはキーごとのレート制限はありません — レート制限はキースコープであり、RPC 呼び出しにはキーがないためです。`realtime.start` は同じユーザー全体の月次クォータゲートを通ります（クォータ枯渇状態でのセッション開始は `-32006` で拒否されます）。`video.create` はジョブ作成時にクォータを検査します。

## ポイント台帳（プリペイド、tier をセーフティネットに）

グループとユーザーはポイントウォレット（`ledger_accounts` — ユーザーごとに
個人ウォレット、グループごとにプール）を持ちます。精算は**ポイント優先**：
計測された各リクエストは引き落とし口座（グループキーならグループプール、
それ以外は個人ウォレット）から
`round(cost_usd × POINTS_PER_USD × メンバー倍率)` の引き落としを試みます —
原子条件付き更新なので、並行引き落としで残高を超えることはありません。
ウォレットで賄えない場合、リクエストは従来どおり tier の月次クォータに
乗ります（後払い、`points = NULL`）。**両方**尽きた場合、REST ゲートは
**402 Payment Required**（`insufficient_credits` / `payment_required`、
`Retry-After` は月末まで）で応答します。RPC パスは `-32006` `QUOTA_ERROR`
の形を維持します。

`POINTS_PER_USD` は環境変数から（既定 100 — 1 ポイント = 1 セント）。
管理面：`group.credits.topup`（プラットフォーム admin）、
`group.credits.allocate`（グループ admin、プール→ウォレットの原子転送）、
`group.credits.balance`、`ledger.self`；`billing.plan` は `points_balance`
と `points_per_usd` を報告します。台帳から精算された使用量行には
`points` と `account_type`（`user` | `group`）が付きます。
